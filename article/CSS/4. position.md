...
<img src="https://user-images.githubusercontent.com/66205484/147172233-88b308a2-eef5-4632-bf1a-3a5027a3ad74.png" alt="image" style="zoom:50%;" />
...
- `top`, `bottom`, `left`, `right` 在绝对定位中是与 `relative` 中不同的。他们是**相对于自身的包含元素的边进行定位的**

我们在静态定位中的 `html` 代码中，他们的**包含元素**其实就是 `body`，所以设置 `left`, `top` 都是相对于**包含元素**来设置的。
...
对绝对定位中的包含元素有异议？其实包含元素取决于绝对定位元素的父元素的 `position` 属性。
...
这时候，我们就会发现，`absolute` 是相对于包含元素的 `relative` 所布局了。
