...
- call 和 apply 两者的区别在于参数。第一个参数均为指定的对象（该函数的执行上下文）
  call 可以有很多参数，后面的参数都是传入函数的值。
  而 apply 只有两个参数，第二个参数是数组，数组就是该函数的参数。
- bind 与两个不同点：bind 方法不会立即执行返回的函数，而 call 和 apply 会直接执行。参数与 call 相同
